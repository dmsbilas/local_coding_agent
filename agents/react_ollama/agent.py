"""
Autonomous LangGraph coding agent powered by local Ollama.

Flow:
  set_output_dir → create_plan → reason → act → observe → … → write_files → END

Same ReAct stack as react_ai_doer, but uses ChatOllama.
Requires: pip install langchain-ollama && ollama serve
"""
...
try:
    ...
    raise ImportError(
        "agents.react_ollama requires langchain-ollama. "
